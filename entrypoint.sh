...
case "$tag_context" in
    *repo*)
        tag=$(git for-each-ref --sort=-creatordate --format '%(refname:lstrip=2)' | grep -E "^v?[0-9]+\.[0-9]+\.[0-9]+\-?\w*" | head -n1)
        pre_tag=$(git for-each-ref --sort=-creatordate --format '%(refname:lstrip=2)' | grep -E "^v?[0-9]+\.[0-9]+\.[0-9]+(-$suffix\.[0-9]+)?$" | head -n1)
        ;;
    *branch*)
